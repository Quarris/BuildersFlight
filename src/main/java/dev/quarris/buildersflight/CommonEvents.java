...
public class CommonEvents {
    ...
    public static void onPlayerLogIn(PlayerEvent.PlayerLoggedInEvent event) {
        ...
        if (event.getPlayer().getPersistentData().contains("flight")) {
            flying = event.getPlayer().getPersistentData().getBoolean("flight");
        }
        PacketHandler.sendTo(new FlightStatePacket(flying), (ServerPlayerEntity) event.getPlayer());
    }
}
